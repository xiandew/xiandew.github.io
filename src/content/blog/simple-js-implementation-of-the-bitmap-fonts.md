...
description: "A step-by-step bitmap font implementation for JavaScript (WeChat Mini Games): generating BMFont assets, converting `.fnt` to JSON, and drawing glyphs on canvas."
pubDate: 2019-03-01
author: "Xiande Wen"
tags: ["javascript", "bitmap-fonts", "wechat", "canvas", "tutorial"]
---

When writing WeChat games, one of the problems I often encountered was using various custom fonts
that are not pre-installed by most user devices. Since WeChat game platform has not given us the privilege
to import those custom font files directly, we have no choice but to use bitmap fonts. I did a lot of
research but there is hardly a concrete implementation of bitmap fonts in JavaScript on the Internet that
could be used by simply importing a bitmap font class. So here is how I implemented it in JavaScript. Feel free
...
To generate bitmap font, you can use the [Bitmap Font Generator](http://www.angelcode.com/products/bmfont/)
which will produce a `.fnt` file and an image containing your chosen characters.

You can find a tutorial on how to use the Bitmap Font Generator [here](http://www.anbsoft.com/content/ezgui/docs/page28/page30/page30.html).
...
There is an awesome tool to convert `XML` to `JSON` online. You can find it
[here](https://www.freeformatter.com/xml-to-json-converter.html)

After getting the `JSON` data, you can place it in a `.js` file and store it as a
constant, which makes it easier to use. Of course, you can place it in a `.json`
file as it should be and import it when needed. I put it in a `.js` file because I had to,
since WeChat platform does not support the file read API of JavaScript.
...
Create a class called `BitmapFont`, which will parse the `JSON` data from string to
JavaScript object and store information including the positions of the characters on the
bitmap image and the image itself. Note that after the font image loads, the passed
...
Create a class called `BitmapText`, which will make use of the information stored in
...
- [Computer fonts > Bitmap fonts](https://en.wikipedia.org/wiki/Computer_font#Bitmap_fonts)
- [Bmfont and how to interpret the .fnt file](https://www.gamedev.net/forums/topic/284560-bmfont-and-how-to-interpret-the-fnt-file/#comment-2785731_wrap)